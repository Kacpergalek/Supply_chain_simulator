...
from data.input_data.agent_data.unfiltered_spatial_data.europe_top_cities import europe_top_cities
from data.input_data.agent_data.unfiltered_spatial_data.world_top_cities import world_top_cities
...
class FactoryManager:
    ...
    """
    def __init__(self):
        self.path = (Path(__file__).parent.parent.parent / "data" / "input_data")
        if (self.path / "agent_data" / "filtered_spatial_data" / "europe_top_cities_factories_filtered.json").exists():
            with open(self.path / "agent_data" / "filtered_spatial_data" / "europe_top_cities_factories_filtered.json") as fp:
                self.cities = json.load(fp)
        else:
            self.cities = europe_top_cities | world_top_cities
    ...
    def initialize_factories(self) -> None:
        ...
        for country in self.cities:
            ...
            for city in self.cities[country]:
                print(f"Querying {city} for factories...")
        ...
        with open(f'{self.path}/agent_data/filtered_spatial_data/world_top_cities_factories_filtered.json', 'w') as fp:
    ...
    def load_from_pickle(self, filename: str) -> pd.DataFrame:
        path = self.path / "factory_data" / f"factories_{filename}.pkl"
        if not path.exists():
