...
class RetailStoreManager:
    ...
    """
    def __init__(self):
        self.json_path = (Path(__file__).parent.parent.parent / "data" / "input_data" / "agent_data" /
                          "filtered_spatial_data" / "world_top_cities_stores_filtered.json")
    ...
    def initialize_stores(self) -> None:
        ...
        for country in self.cities:
            ...
            for city in self.cities[country]:
                print(f"Querying: {city} for retail stores...")
    ...
    def save_to_pickle(self, filename: str) -> None:
        path = Path(__file__).parent.parent.parent / "data" / "input_data" / "store_data"
    ...
    def load_from_pickle(self, filename: str) -> pd.DataFrame:
        path = Path(__file__).parent.parent.parent / "data" / "input_data" / "store_data" / f"stores_{filename}.pkl"
        if not path.exists():
